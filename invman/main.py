from flask import Blueprint, render_template, request, url_for, g, redirect, session, jsonify
...
def product_list(id):
    ...
    db = get_db()
    product_list = db.execute('SELECT product_id, product_name, quantity FROM product WHERE for_business = ? AND quantity > 0', (id,)).fetchall()
    return product_list

def get_product(location, product_id, quantity):
    """ Used by the movement route to get a product from a location. """
    # Gets the product from the product table or from the locations.
    ...
    if location == 'product_factory':
        ...
        ogquantity = db.execute('SELECT quantity FROM product WHERE product_id = ? AND for_business = ?', (product_id, b_id,)).fetchone()[0]
        newquantity = ogquantity - quantity
        if int(newquantity) < 0:
            raise Exception("Invalid quantity.")
        query = 'UPDATE product SET quantity = ? WHERE product_id = ? AND for_business = ?'
        db.execute(query, (newquantity, product_id, b_id))
        p = db.execute('SELECT product_id FROM product WHERE for_business = ? AND product_id = ?', (b_id, product_id,)).fetchone()
        product_array = list(p)
        product_array.append(quantity)
        db.commit()
        return product_array
    else:
        ogquantity = db.execute('SELECT qty FROM warehouse WHERE loc_id = ? AND prod_id = ? AND b_id = ?', (location, product_id, b_id,)).fetchone()[0]
        newquantity = ogquantity - quantity
        if int(newquantity) < 0:
            raise Exception("Invalid quantity.")
        query = 'UPDATE warehouse SET qty = ? where loc_id = ? AND prod_id = ? AND b_id = ?'
        db.execute(query, (newquantity, location, product_id, b_id,))
        p = db.execute('SELECT prod_id FROM warehouse WHERE prod_id = ? AND loc_id = ? AND b_id = ?', (product_id, location, b_id,)).fetchone()
        if int(newquantity) == 0:
            db.execute('DELETE FROM warehouse WHERE b_id = ? AND prod_id = ? AND loc_id = ?', (b_id, product_id, location,))
        product_array = list(p)
        product_array.append(quantity)
        db.commit()
        return product_array
...
def set_product(location, product_array):
    ...
    if location != "Move out":
        product_exists = db.execute('SELECT * FROM warehouse WHERE prod_id = ? AND loc_id = ? AND b_id = ?', (product_id, location, b_id)).fetchone()
        if product_exists:
            ogquantity = db.execute('SELECT qty FROM warehouse WHERE loc_id = ? AND prod_id = ? AND b_id = ?', (location, product_id, b_id,)).fetchone()[0];
            newquantity = ogquantity + quantity
            query = 'UPDATE warehouse SET qty = ? WHERE loc_id = ? AND prod_id = ? AND b_id = ?'
            ...
            db.execute('INSERT INTO warehouse (b_id, prod_id, qty, loc_id) values (?, ?, ?, ?)', (b_id, product_id, quantity, location))
            db.commit()
...
def balance_quantity(quantity, product_id, location):
    ...
    db = get_db()
    if location == 'product_factory':
        ogquantity = db.execute('SELECT quantity from product WHERE product_id = ?', (product_id,)).fetchone()
...
def products():
    """ The product route. """
    # Display available products on the page. (Done.)
    # Add products to the product table. (Done.)
    # Update existing products in the product table. (Done.)
    ...
    if request.method == 'POST':
        if 'submit_product' in request.form:
            try:
                ...
                if prod_qty < 0:
                    raise Exception('Invalid quantity.')
                db.execute('INSERT INTO product (product_id, product_name, quantity, for_business) values (?, ?, ?, ?)', (prod_id, prod_name, prod_qty, b_id,))
                db.commit()
                ...
                if "UNIQUE constraint failed" in str(e): 
                    ...
                    error = str(e)
            return render_template('products.html', title='Products', products=product_list(b_id), error=error)
        if 'update_product' in request.form:
            try:
                ...
                if prod_name:
                    ...
                    db.commit()
                balance_quantity(prod_qty, prod_selected, location="product_factory",)
                db.commit()
            except Exception as e:
    ...
    return render_template('products.html', products=prod_list, title='Products', error=error)
...
def delete_loc(lid):
    ...
    db = get_db()
    b_id = session.get('user_id')
    db.execute('DELETE FROM location WHERE location_id = ? AND for_business = ?', (lid, b_id,))
    db.commit()
...
def locations():
    """ The location route. """
    # Display all available locations on the page. (Done.)
    # Add locations to the location table. (Done.)
    # Update locations in the location table. (Done.)
    ...
    if request.method == 'POST':
        if 'submit_location' in request.form:
            try:
                ...
                db.execute('INSERT INTO location (location_id, location_name, for_business) values(?, ?, ?)', (loc_id, loc_name, b_id,))
                db.commit()
            except Exception as e:
                if 'UNIQUE constraint failed:' in str(e):
                    ...
                    error = str(e)
            location_list = db.execute('SELECT location_id, location_name FROM location where for_business = ?', (b_id,)).fetchall()
            return render_template('locations.html', title='Locations', locations=location_list, error=error)
        if 'update_location' in request.form:
            try:
                ...
                db.execute('UPDATE location SET location_name = ? WHERE location_id = ?', (new_locname, loc_selected,))
                db.commit()
            except Exception as e:
                error = str(e)
            location_list = db.execute('SELECT location_id, location_name FROM location where for_business = ?', (b_id,)).fetchall()
...
def loadproducts(lid):
    ...
    if lid == 'Product Factory':
        ...
        for products in warehouses:
            product_name = db.execute('SELECT product_name FROM product WHERE product_id = ? AND for_business = ?', (products['prod_id'], b_id,)).fetchone()
...
def getquantity(prd, loc):
    ...
    if loc == 'Product Factory':
        if prd != 'None':
            q = db.execute('SELECT quantity FROM product WHERE product_id = ? AND for_business = ?', (prd, b_id,)).fetchone()
            qty['qty'] = str(q['quantity'])
        else:
            pass
    else:
        q = db.execute('SELECT qty FROM warehouse WHERE prod_id = ? AND b_id = ? AND loc_id = ?', (prd, b_id, loc,)).fetchone()
        qty['qty'] = str(q['qty'])
    return qty
...
def products_at_locations():
    ...
    for ids in locs:
        l = []
        prods = db.execute('SELECT prod_id, qty FROM warehouse where b_id = ? AND loc_id = ?', (b_id, ids,)).fetchall()
        locname = db.execute('SELECT location_name FROM location WHERE location_id = ? AND for_business = ?', (ids, b_id,)).fetchone()['location_name']
        for data in prods:
            prodname = db.execute('SELECT product_name FROM product WHERE for_business = ? AND product_id = ?', (b_id, data['prod_id'],)).fetchone()['product_name']
            l.append([data['prod_id'] + ' ' + prodname, data['qty']])
        warehouse[locname] = l
    return warehouse

def logmovements(movement_id, from_location, to_location, prod_id, qty,):
    db = get_db()
    b_id = session.get('user_id')
    ...
    if to_location == 'Move out':
        to_location = 'MO'
    db.execute('INSERT INTO movement (movement_id, from_location, to_location, prod_id, qty, b_id)'
                'VALUES (?, ?, ?, ?, ?, ?)',
...
def movements():
    """ Movement route. """
    # Move products to a location. (Done)
    # Move products from one location to another. (Done)
    # Remove products from a location. (Done)
    # Do the above three steps while keeping track of the quantity for that product. (Done)
    # Create a timestamp when each movement occurs. (Done)
    # Update the movement table accordingly. (Done)
    # Table warehouse refers to the products which are currently at a location.
    # Update warehouse accordingly. (Done)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            prod_list = product_list(b_id)
            move_to = db.execute('SELECT location_id, location_name FROM location WHERE for_business = ?', (b_id,)).fetchall()
...
def movementlogs():
    ...
    business_name = db.execute('SELECT business_name FROM business WHERE business_id = ?', (b_id,)).fetchone()

    return render_template('movementlogs.html', logtable=logtable, business_name=business_name, title="Logs")
